...
export function initMap(){state.map=L.map("map",{zoomControl:false,attributionControl:false,preferCanvas:true});L.tileLayer("https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png",{subdomains:"abcd",maxZoom:20}).addTo(state.map);state.map.setView([55.6761,12.5683],12);}
export function drawRoute(geometry){if(!state.map||!Array.isArray(geometry))return;clearRoute();const latlngs=geometry.map(p=>[p[1],p[0]]);state.routeGlow=L.polyline(latlngs,{color:"#68c4ff",weight:22,opacity:.20,lineCap:"round",lineJoin:"round"}).addTo(state.map);state.routeDoneLine=L.polyline([],{color:"#173554",weight:9,opacity:.72,lineCap:"round",lineJoin:"round"}).addTo(state.map);state.routeAheadLine=L.polyline(latlngs,{color:"#2b91ff",weight:10,opacity:1,lineCap:"round",lineJoin:"round"}).addTo(state.map);state.routeLine=state.routeAheadLine;state.map.fitBounds(state.routeAheadLine.getBounds(),{paddingTopLeft:[55,260],paddingBottomRight:[55,260],animate:true});}
export function updateRouteHighlight(progress){if(!state.routeData?.geometry||!state.routeAheadLine||!state.routeDoneLine)return;const g=state.routeData.geometry,index=Math.max(1,Math.min(g.length-1,progress?.segmentIndex||1));state.routeDoneLine.setLatLngs(g.slice(0,index+1).map(p=>[p[1],p[0]]));state.routeAheadLine.setLatLngs(g.slice(index).map(p=>[p[1],p[0]]));}
