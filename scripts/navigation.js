...
function updateUi(cur,p){els.driveRemainingDistance.textContent=formatDistance(p.remainingMeters);els.driveRemainingTime.textContent=formatDuration(p.remainingSeconds);els.driveEtaValue.textContent=new Date(Date.now()+p.remainingSeconds*1000).toLocaleTimeString("da-DK",{hour:"2-digit",minute:"2-digit"});const step=findStep(p.alongMeters);if(step){const d=Math.max(0,step.endDistance-p.alongMeters);els.nextTurnDistance.textContent=d<8?"Nu":formatDistance(d);els.nextTurnInstruction.textContent=instruction(step);els.nextTurnRoad.textContent=step.name||"";els.turnIcon.textContent=icon(step)}const rec=getGreenWaveRecommendation(cur);els.currentSpeedValue.textContent=Math.round(cur.speed); if(els.topCurrentSpeed) els.topCurrentSpeed.textContent=Math.round(cur.speed);els.speedLimitValue.textContent=state.currentMaxSpeed||"?";els.recommendedSpeedValue.textContent=rec.speedKmh||"--";}
