...
const SETTINGS_KEY = "gw_settings_v2";
const HISTORY_KEY = "gw_history_v2";
const PRICE_HISTORY_KEY = "gw_price_history_v2";
...
function bindEvents() {
  ...
  els.startNavBtn?.addEventListener("click", startNav);
  els.stopNavBtn?.addEventListener("click", stopNav);
  ...
  els.exitNavOverlayBtn?.addEventListener("click", stopNav);
}
...
function applyTranslations() {
  ...
  if (els.openFuelHistoryBtn) els.openFuelHistoryBtn.textContent = t("history");
  if (els.gpsStatusChip) els.gpsStatusChip.textContent = t("gps");
  if (els.navStatusChip) els.navStatusChip.textContent = t("nav");
}
...
function renderAutocomplete(items) {
  ...
  items.forEach(item => {
    const title = item.name || item.address?.road || item.display_name?.split(",")[0] || "Adresse";
...
async function calculateRoute() {
  try {
    ...
    state.destination = state.selectedAutocompleteItem || await geocode(destinationText);
    ...
    drawRoute(state.routeData.geometry.map(c => [c[1], c[0]]));
    fitRoute();
...
function updateFuelBox() {
  ...
  els.fuelContent.innerHTML = `
    ...
    <div class="fuel-meta">Omvej: ${formatDist(best.extraDetourMeters)}</div>
    <div class="fuel-meta">Fra rute: ${formatDist(best.distanceToRouteMeters)}</div>
...
function getFuelCandidates() {
  ...
  return state.osmFuelStations
    ...
    .map(s => {
      const d = distanceToRouteMetersFromGeometry(
        { lat: s.lat, lng: s.lng },
        state.routeData.geometry.coordinates || state.routeData.geometry
      );

      return {
        ...s,
        distanceToRouteMeters: d,
        extraDetourMeters: d * 2
      };
    })
  ...
  candidates.forEach((station, index) => {
    ...
    const marker = L.marker([station.lat, station.lng], { icon })
      ...
      .bindPopup(`
        ...
        Omvej: ${formatDist(station.extraDetourMeters)}<br>
  ...
  els.fuelListContent.innerHTML = sorted.map((s, i) => `
    <article class="fuel-list-item">
      ...
      <div class="fuel-list-meta-grid">
        <div class="fuel-list-meta">Omvej<br><strong>${formatDist(s.extraDetourMeters)}</strong></div>
        <div class="fuel-list-meta">Fra rute<br><strong>${formatDist(s.distanceToRouteMeters)}</strong></div>
...
function dedupeStations(stations) {
  const result = [];
  stations.forEach(s => {
    if (!result.some(x => distance(s.lat, s.lng, x.lat, x.lng) < 35)) result.push(s);
  });
  return result;
}
