export default async function handler(req,res){
  ...
  if(String(req.query.avoid||"").toLowerCase().includes("ferries"))return res.status(400).json({error:"Undgå færger understøttes ikke af den aktuelle rutekilde",unsupported:{avoid:"ferries",provider:"OSRM public route API"}});
  try{const url=`https://router.project-osrm.org/route/v1/driving/${fromLng},${fromLat};${toLng},${toLat}?overview=full&geometries=geojson&steps=false&alternatives=true&annotations=distance,duration,speed`;const r=await fetch(url,{headers:{Accept:"application/json"}});if(!r.ok)return res.status(502).json({error:`OSRM HTTP ${r.status}`});return res.status(200).json(await r.json());}catch(e){return res.status(500).json({error:e.message});}
}
