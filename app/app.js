const GREENWAVE_VERSION="v1.18-ev-planner";
const SKEY="greenwave_dk_settings_v2",HKEY="greenwave_dk_history_v2",EKEY="greenwave_dk_ev_planner_v1";
const EV_PLANNER_PROFILES={renaultMeganeETech:{id:"renaultMeganeETech",name:"Renault Megane E-Tech EV60",usableBatteryKwh:60,defaultConsumptionKwh100:17,notes:"60 kWh brugbar batterikapacitet. Forbrug er justerbart."}};
...
function initEvPlannerUi(){
  ...
  card.innerHTML=`<header><h2>EV Planner v1</h2><button id="evUseRouteBtn" type="button">Brug rute</button></header><div class="ev-profile-note">Bilprofil: Renault Megane E-Tech EV60 · 60 kWh brugbar · forbrug kan justeres.</div><div class="ev-planner-grid"><label>Bilprofil<select id="evProfileSelect"><option value="renaultMeganeETech">Renault Megane E-Tech</option></select></label><label>Start-SoC %<input id="evStartSocInput" type="number" min="0" max="100" step="1"></label><label>Minimum-SoC %<input id="evMinSocInput" type="number" min="0" max="50" step="1"></label><label>Rutedistance km<input id="evRouteDistanceInput" type="number" min="0" step="0.1"></label><label>Forbrug kWh/100 km<input id="evConsumptionInput" type="number" min="5" max="35" step="0.1"></label></div><div id="evPlannerSummary" class="ev-planner-summary"></div><div id="evPlannerSuggestions" class="ev-planner-suggestions"></div>`;
  ...
  syncEvPlannerUi();
  [els.evProfileSelect,els.evStartSocInput,els.evMinSocInput,els.evRouteDistanceInput,els.evConsumptionInput].forEach(el=>on(el,"input",()=>{saveEvPlannerSettings();renderEvPlanner();}));
  [els.evProfileSelect,els.evStartSocInput,els.evMinSocInput,els.evRouteDistanceInput,els.evConsumptionInput].forEach(el=>on(el,"change",()=>{saveEvPlannerSettings();renderEvPlanner();}));
  on(els.evUseRouteBtn,"click",()=>{syncEvPlannerRouteDistance(true);saveEvPlannerSettings();renderEvPlanner();});
}
function defaultEvPlannerSettings(){const profile=EV_PLANNER_PROFILES.renaultMeganeETech;return{profileId:profile.id,startSoc:80,minSoc:15,routeDistanceKm:"",consumptionKwh100:profile.defaultConsumptionKwh100};}
...
function evPlannerInputs(){const profile=EV_PLANNER_PROFILES[els.evProfileSelect?.value]||EV_PLANNER_PROFILES.renaultMeganeETech;const routeInput=numPlannerInput(els.evRouteDistanceInput,NaN);const routeDistanceKm=Number.isFinite(routeInput)&&routeInput>0?routeInput:(state.route?.distance?Number(state.route.distance)/1000:0);return{profile,startSoc:Math.max(0,Math.min(100,numPlannerInput(els.evStartSocInput,80))),minSoc:Math.max(0,Math.min(80,numPlannerInput(els.evMinSocInput,15))),routeDistanceKm,consumptionKwh100:Math.max(5,Math.min(35,numPlannerInput(els.evConsumptionInput,profile.defaultConsumptionKwh100)))};}
function calculateEvPlanner(){const v=evPlannerInputs();const energyNeededKwh=v.routeDistanceKm*v.consumptionKwh100/100;const socUsed=v.profile.usableBatteryKwh>0?energyNeededKwh/v.profile.usableBatteryKwh*100:0;const arrivalSoc=v.startSoc-socUsed;const safeEnergyKwh=Math.max(0,(v.startSoc-v.minSoc)/100*v.profile.usableBatteryKwh);const safeRangeKm=v.consumptionKwh100>0?safeEnergyKwh/v.consumptionKwh100*100:0;const missingKwh=Math.max(0,(v.minSoc-arrivalSoc)/100*v.profile.usableBatteryKwh);return{...v,energyNeededKwh,socUsed,arrivalSoc,safeRangeKm,missingKwh,needsCharge:arrivalSoc<v.minSoc};}
function renderEvPlanner(){if(!els.evPlannerSummary||!els.evPlannerSuggestions)return;const plan=calculateEvPlanner();if(!plan.routeDistanceKm){els.evPlannerSummary.innerHTML=`<div class="ev-planner-empty">Indtast rutedistance eller beregn en rute for at planlægge batteriet.</div>`;els.evPlannerSuggestions.innerHTML="";return;}const arrival=Math.max(0,Math.min(100,plan.arrivalSoc));const status=plan.needsCharge?"Ladestop anbefales":"Batteriet rækker";const level=plan.needsCharge?"warn":"ok";els.evPlannerSummary.innerHTML=`<div class="ev-planner-kpis"><div><span>Forventet ankomst</span><strong>${formatSoc(arrival)}</strong></div><div><span>Minimum</span><strong>${formatSoc(plan.minSoc)}</strong></div><div><span>Forbrug</span><strong>${plan.energyNeededKwh.toFixed(1).replace(".",",")} kWh</strong></div></div><div class="ev-planner-status" data-level="${level}">${status}. ${plan.needsCharge?`Mangler ca. ${plan.missingKwh.toFixed(1).replace(".",",")} kWh for at ankomme med ${formatSoc(plan.minSoc)}.`:`Estimeret reserve ved ankomst er ca. ${(arrival-plan.minSoc).toFixed(0)} procentpoint.`}</div><div class="ev-planner-note">${esc(plan.profile.name)} · ${plan.profile.usableBatteryKwh} kWh brugbar · ${plan.consumptionKwh100.toFixed(1).replace(".",",")} kWh/100 km.</div>`;renderEvPlannerSuggestions(plan);}
...
function ensureEvPlannerStyles(){if(document.getElementById("ev-planner-style"))return;const style=document.createElement("style");style.id="ev-planner-style";style.textContent=".ev-planner{margin-top:12px}.ev-profile-note,.ev-planner-note,.ev-planner-empty{color:var(--muted);font-size:13px}.ev-planner-grid{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:10px;margin-top:12px}.ev-planner-grid label{display:block;color:var(--muted);font-size:12px;text-transform:uppercase}.ev-planner-grid input,.ev-planner-grid select{width:100%;margin-top:6px;background:#111f33;border:1px solid #31425d;border-radius:14px;padding:11px}.ev-planner-kpis{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-top:12px}.ev-planner-kpis div,.ev-planner-status{background:#101d2f;border:1px solid var(--line);border-radius:16px;padding:12px}.ev-planner-kpis span{display:block;color:var(--muted);font-size:11px;text-transform:uppercase}.ev-planner-kpis strong{display:block;margin-top:3px;font-size:20px;color:#fff}.ev-planner-status{margin-top:10px;color:#dce8f8}.ev-planner-status[data-level=ok]{border-color:rgba(57,228,123,.55)}.ev-planner-status[data-level=warn]{border-color:rgba(255,194,102,.65);color:#ffe3ad}.ev-planner-suggestions h3{margin:14px 0 4px;color:var(--muted);font-size:13px;text-transform:uppercase}@media(max-width:720px){.ev-planner-grid{grid-template-columns:1fr 1fr}.ev-planner-grid label:first-child{grid-column:1/-1}.ev-planner-kpis{grid-template-columns:1fr}}";document.head.appendChild(style);}
