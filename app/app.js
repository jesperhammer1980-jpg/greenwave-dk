const GREENWAVE_VERSION="v1.16-charging-providers";
...
function renderCharging(d){setFuelPanelMode();const all=state.stations||[],visible=visibleChargingPoints(),priced=visible.filter(p=>isValidChargePrice(p.priceKwh)).length;if(!all.length){const raw=d?chargingDebugValue(d,"rawElements"):"?";const norm=d?chargingDebugValue(d,"normalizedChargingPoints"):"?";els.fuelSummary.textContent=d?`0 ladepunkter. Debug: raw=${raw}, norm=${norm}, returned=${d?.counts?.returned??0}, bbox=${JSON.stringify(d?.input?.routeBbox||d?.debug?.overpass?.bbox||{})}`:"Beregn en rute først.";els.fuelList.innerHTML="";return;}const filtered=visible.length!==all.length?` ${visible.length} vist efter filter.`:"";els.fuelSummary.textContent=`${all.length} ladepunkter langs ruten.${filtered} ${priced} med kendt ladepris. Kilder: ${chargingSourceStatusSummary(d)}.`;els.fuelList.innerHTML=visible.slice(0,20).map(p=>{const hasPrice=isValidChargePrice(p.priceKwh);const price=hasPrice?formatChargePrice(p.priceKwh):"Pris ikke tilgængelig";const quality=p.priceQuality==="operator-guidance"?"Vejledende operatørpris":p.priceQuality==="station-specific"?"Stationsspecifik pris":"";const meta=[p.provider||"Ukendt leverandør",p.powerKw?`${p.powerKw} kW`:"",p.connectorType||"",`${fmtDist(p.distanceAlongRoute)} langs ruten`,`${fmtDist(p.distanceToRoute)} fra ruten`,hasPrice&&p.priceSource?`Pris fra ${p.priceSource}`:"",quality].filter(Boolean).join(" · ");return `<article class="fuel-item"><div class="fuel-title"><span>${esc(p.name||"Ladepunkt")}</span><span class="fuel-price">${esc(price)}</span></div><div class="fuel-meta">${esc(meta)}</div><a target="_blank" href="https://www.google.com/maps/search/?api=1&query=${p.lat},${p.lng}">Åbn i Google Maps</a></article>`;}).join("");}
...
function chargingSourceStatusSummary(d){const src=Array.isArray(d?.sources)?d.sources:[];if(!src.length)return"OSM";return src.map(s=>`${s.name||s.id}:${s.ok?"OK":s.configured===false?"ikke konfigureret":"FEJL"}`).join(" · ");}
